...
стала первой, а последний элемент массива - первым. Путь к базе данных
будет передаваться как аргумент при запуске. Готовый проект
нужно собрать в исполняемый файл:
```shell
cargo build --release
```
Он будет находиться в `Rust/target/release`. Теперь, пора приступать
к реализации части на Python.
...
получения. Путь к БД будет передаваться как аргумент при запуске, как
на Rust.
...
работающему на порту `8103`. Сразу соберем проект в исполняемый файл:
```shell
...
```
Мы указываем только имя, так как путь мы будем задавать аргументом
`-Djava.library.path=""` при запуске программы.
Теперь мы можем собрать Java-проект в исполняемый
JVM файл `.jar`. Для этого запускаем 2 задачи maven: сначала `clean`,
затем `install`. В директории `target` появится `.jar` файл.
...
cd Translator
# Запускаем Java gRPC-сервер
java -Djava.library.path="C++/" -jar "Java/target/Java-1.0.jar"
# Запускаем Golang WebSocket-сервер
"Golang/Golang.exe"
# Запускаем программу на Python
py Python/main.py  "SQL/messages.db"
# Запускаем Web-сервер Rust
"Rust/target/release/Translator.exe"  -- "SQL/messages.db"
# Открываем веб-страницу
"JavaScript/index.html"
```
Теперь тестируем. Вводим строку и элементы массива. Они должны
циклично сдвинуться вправо 5 раз (на Rust, Python, Go, Java и C++) и
вернуться в виде единой строки.

<h5 align="right">Результат на веб-странице</h5>
<div align="center">
    <img src="assets/result.png" alt="web-page"/>
</div>
Все работает! Но для реальных проектов лучше выбирать конкретный способ
