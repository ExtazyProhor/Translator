...
Для того, чтобы взаимодейстовать с SQLite напрямую или через
...
`LIB`. Для создание базы данных, переходим в каталог с
...
`cargo new Rust`. В файл `Cargo.toml` добавляем зависимости
упомянутых библиотек:
```toml
[dependencies]
actix-web = "4"
